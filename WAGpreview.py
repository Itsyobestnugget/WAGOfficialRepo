...
cmd_helpenabled = 1
cmd_infoenabled = 1
cmd_enteradminenabled = 0
cmd_systeminfoenabled = 1
cmd_whoamienabled = 1
print("Welcome to WAG File Manager!")
while running == 1:
    i = input("user: ")
    if i == "dir":
        if cmd_direnabled == 1:
    ...
    if i == "whoami":
        if cmd_whoamienabled == 1:
            ...
            print("command disabled")
    if i == "ashell":
        fullink = "https://nuggetplayzyay.wixstudio.io/oscario/"
        print(f"WAG CLI Preview doesnt contain an AWAG SHELL which allows system usage and custom called commands like the one in your normal shell. to use the AWAG SHELL, Download the WAG Full version from here '{fullink}")
    else:
        fullink = "https://nuggetplayzyay.wixstudio.io/oscario/"
        print(f"WAG Preview doesnt allow commands besides basic commands. to use advanced commands, Download the WAG Full version from here '{fullink}")